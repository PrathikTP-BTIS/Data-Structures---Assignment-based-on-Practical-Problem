...
// Node structure to define the structure of a node in a binary tree
typedef struct Node 
{
    int data;
...
// Function to delete a node with given key from binary tree
Node* deletion(Node* root, int key) 
{
    ...
    // Perform level order traversal to find deepest node (temp) and node to be deleted (key_node)
    while (front != rear) 
    {
        temp = queue[front++];
